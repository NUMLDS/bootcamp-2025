...
author: "Alice Zhao"
date: "`r paste('Fall', format(Sys.Date(), '%Y'))`"
output: 
...
ggplot(gapminder07) + 
  ...
  labs(title = "Relationship between GDP per capita and population in 2007", x = "Logged population", y = "Logged GDP per capita")
```
...
> - **Labels**: titles, axis labels, subtitles, captions, etc.
> - **Scales**: scale_alpha, scale_color, scale_fill, etc.
> - **Themes**: theme_grey, theme_bw, theme_dark, theme_light, etc.
> - **Theme layer**: angle of labels, etc.
> - **Coordinate system adjustments**: coord_flip, coord_transform, etc.
...
## Themes example: Imports over time
...
## Theme layer example: Imports over time
...
## Coordinate system adjustment example: coord_flip()
...
## Customizing with scale_color layer
...
## Example: Energy use by day (position fill)
...
> 1. Supply long_merged_energy_regroup data frame to `ggplot()`.
> 2. Two choices!
>     1. Add `geom_line()` and `geom_point()` with `shape=group`.
>     2. Add `geom_line()` with `linetype=group`.

## Example: Energy source group, by day (shape = group)
...
## Example: Energy source group, by day (linetype = group)
...
## Scale example: Life expectancy over GDP
...
## Example (version 2): Comparing generation patterns
